...
func write(items []Item, db *sql.DB) error {
	for _, item := range items {
		deleteUser(db, item.ID)
	}
	err := createTable(db)
	if err != nil {
		return err
	...
	for _, item := range items {
		_, exec := db.Exec("INSERT INTO items (id, name, phone, account, address, isOwner) VALUES (?, ?, ?, ?, ?, ?)", item.ID, item.Name, item.Phone, item.Account, item.Address, item.isOwner)
		if exec != nil {
			return exec
